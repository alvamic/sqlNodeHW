          ...
          connection.query(query + connection.escape(userInput), function (err, res) {
            ...
            for (var i = 0; i < res.length; i++) {
              ...
              if (database > res[i].stock_quantity) {
                ...
                connection.query('UPDATE products SET stock_quantity' - connection.escape(database) + 'connection WHERE product_name=' + connection.escape(userInput), function (err, res) {
                  if (err) throw err;
                 })
              }
            }
